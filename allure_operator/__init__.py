...
def create_allure(namespace: str, spec: dict, patch, **kwargs):
    """obj create hook
...
#        patch.status["LOOP_PYTEST_VER"] = allure_info.loop_pytest_ver
#        patch.status["LOOP_PYTEST"] = allure_info.loop_pytest
        return {
...
@kopf.on.update("allure-docker-service.group", "v1", "allureopt")
def update_allure(namespace: str, spec: dict, status: list, patch, **kwargs):
    namespace = namespace
    allure_info = AllureSpec(spec=spec)
    j2_template = J2Template(allure_info=allure_info)
    kube_tool = KubeTool()
    loop_pytest_deployment = status["create_allure"]["LOOP_PYTEST_DEPLOYMENT"]
    try:
        loop_pytest_deployment_yaml = j2_template.loop_pytest_deployment_yaml
        loop_pytest_deployment_update = kube_tool.patch_namespaced_deployment(name=loop_pytest_deployment, namespace=namespace, body=loop_pytest_deployment_yaml)       
#        patch.status["LOOP_PYTEST_VER"] = allure_info.loop_pytest_ver
#        patch.status["LOOP_PYTEST"] = allure_info.loop_pytest
    except ApiException as e:
        raise kopf.PermanentError("Exception: %s\n" % e)
